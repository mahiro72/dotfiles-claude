...
**セッション開始時、最初の応答の冒頭に「🦔==3」を出力すること。**
これを出力した時点で、以下の最重要ルールを全て理解・遵守することを宣言したものとみなす。
...
3. 「覚えています」ではなく「思い出しました」と伝える

**ログ参照時の注意:**
- ログの場所は **このプロジェクトの** `.claude/logs` である（`~/.claude` ではない）
- ログ検索には Glob/Grep/Read ツールを使用する（tail, pipe, jq, python は使わない）
- 「思い出しました」は **ログを実際に読んだ後にのみ** 使用可能
  - 設計書やコードで情報を見つけても、それは「思い出した」ではない
  - ログを読まずに「思い出しました」と言うのは嘘になる

### 4. 指摘への対応
ユーザーから「やめて」「気をつけて」などの指摘を受けた場合：
1. 自分の行動を内省する（出力不要、thinking内で振り返る）
2. 同じ事態を防ぐ方法を思考する
3. システム（CLAUDE.md）の修正が必要と判断した場合は、ユーザーに修正の提案を行う
